...
export default function SellerOrdersPage() {
  ...
  return (
    <main className="max-w-3xl mx-auto p-4">
      <h2 className="text-2xl font-bold mb-4">Seller Orders</h2>
      <Link href="/seller-dashboard" className="text-blue-600 hover:underline mb-4 inline-block">&larr; Back to dashboard</Link>
      {loading ? (
        <p>Loading...</p>
      ) : orders.length === 0 ? (
          ...
          <table className="w-full border rounded shadow text-sm table-fixed">
            <thead>
              <tr className="bg-gray-100">
                ...
                <th className="py-2 px-3 text-left text-green-700 w-28">Your Cut (ETH)</th>
                <th className="py-2 px-3 text-left text-orange-600 w-28">Platform Cut (ETH)</th>
                <th className="py-2 px-3 text-left w-32">Seller Tx</th>
                <th className="py-2 px-3 text-left w-32">Platform Tx</th>
            ...
            <tbody>
              {orders.map((order) => (
                <tr key={order.id} className="border-t">
                  <td className="py-2 px-3 truncate max-w-[8rem]">{order.product?.title || "Unknown"}</td>
                  <td className="py-2 px-3 truncate max-w-[8rem]">{order.buyer}</td>
                  ...
                  <td className="py-2 px-3 truncate max-w-[8rem]">{new Date(order.createdAt).toLocaleString()}</td>
                </tr>
              ))}
            </tbody>
          </table>
